...
require 'taurus/core/custom_fixtures'
...
namespace :db do
  ...
  task :load_file , [:file, :dir] => :environment do |t, args|
    ...
    if %w{.csv .yml}.include? file.extname
      ...
      Taurus::Core::Fixtures.create_fixtures(args.dir, file.to_s.sub(file.extname, ""))
    elsif file.exist?
  ...
  task :bootstrap  do
    ...
    load_defaults  = taurus::Country.count == 0
    ...
    if Rails.env.production? and taurus::Product.count > 0
    ...
    if load_sample
      ...
      Rake::Task["taurus_sample:load"].invoke
    end
