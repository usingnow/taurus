            ...
            property.name
	    		...
	    		input += content_tag :div, :class => "controls" do
			      ...
			      when 1
			        ...
			        :class => "numeric integer optional", :step => 1, :type => "number"
			      when 2
			      	text_field_tag :"string_value[#{property.id}]", property_value ? property_value.string_value : 0,
			      	 :class => "numeric decimal optional", :step => "any", :type => "number"
			      when 3
			      	date_select(:custom_property_value, :"string_value[#{property.id}]", 
			      		{ :default => property_value ? property_value.string_value.to_date : Time.now }, { :class => "date optional" })
			      when 9
